...
'''
# Needs newer floating point division, otherwise, integer points will cause incorrect answers
...
import numpy as np  # Adding support for numpy arrays
import math
import sys
import operator
import types
...
class Vector2:
    ...
    def __nonzero__(self):
        # CARLOS: for some reason float64 that comes from numpy fails here, so added float type casting
        return float(self.x) != 0 or float(self.y) != 0
...
class Vector3:
    ...
    def __nonzero__(self):
        # CARLOS: for some reason float64 that comes from numpy fails here, so added float type casting
        return float(self.x) != 0 or float(self.y) != 0 or float(self.z) != 0
...
def _get_closest_point_on_line(point, line):
    '''
    Finds the closes point on a line from another point (it works for both 2D and 3D geometries)
    such that the normal distance between a point and the line is perpendicular.
    In the case of a line segment, it will get extended to form a line.

    @param point: A point object in euclidean space
    @param line: The line object in euclidean space on which the orthonormal (closest) point will be found

    @return The closest point on the line from the point
    '''

    w = point - line.p1
    c1 = line.v.dot(w)

    c2 = line.v.dot(line.v)
    ratio = float(c1) / float(c2)
    return line.p1 + ratio * line.v

def _get_closest_point_in_segment(point, segment):
    '''
    @brief Finds closest point within a line segment from a point (it works for both 2D and 3D geometries).
    Results for 2D point/lines are the same when using the original connect() method. However, this is not the case for 3D.
    The normal distance between a point and a line must be perpendicular. However, for a line segment that is not perpendicular to the point,
    it will return the closest point on the segment.

    @param point: A point object in euclidean space
    @param segment: The line segment object in euclidean space on which the closest point will be found

    @return The closest point in the line segment from the point
    '''
    w = point - segment.p1
    c1 = segment.v.dot(w)
    if c1 <= 0.0:
    ...
    c2 = segment.v.dot(segment.v)
    if c2 <= c1:
        return segment.p2

    ratio = float(c1) / float(c2)
    return segment.p1 + ratio * segment.v

def _distance_normal(point, other):
    '''
    @brief Computes the shortest distance from a point to another Geometry entity.
    The shortest distance between a line an a point is on the line perpendicular to the line passing through the point
    The normal distance between a point and a line must be perpendicular.
    In the case of a line segment, it will get extended to form a line.

    @param point: A point object in euclidean space
    @param other: A target Geometry in euclidean space to which the closest point will be found to return its distance to it from the point

    @return The shortest distance between a point and another geometric object.

    '''
    if isinstance(other, (Line2, Line3)):
        closest_point_to_line = point.get_closest_point_on_line(other)
        return(point.distance(closest_point_to_line))
    else:
...
def _intersect_point2_line2(P, L):
    if P.is_collinear(L):
        if isinstance(L, (LineSegment2, Ray2)):
            # WISH: test if point is between ray or segment
            pass  # return
        return True
    else:
        return False
...
# An infinite "Line" is being intersected with the circle, but this is not true for line segments!
# WISH2: define _intersect_linesegment2_circle(L, C) so it only intersecting points result
...
def _intersect_circle_circle(A, B):
    ...
    EPSILON = sys.float_info.epsilon  # Added by Carlos since EPSILON was undefined
    # EPSILON = 1e-8

    if p1.distance(p2) > EPSILON:
        return (p1, p2)
    else:  # It's the same point (or tangent circles)
        return p1
...
class Point2(Vector2, Geometry):
    ...
    def intersect(self, other):
        if isinstance(other, Line2):
            return self._intersect_line2(other)
    ...
    def distance_normal(self, other):
        return _distance_normal(self, other)

    def get_closest_point_on_line(self, line):
        return  _get_closest_point_on_line(self, line)

    def get_closest_point_in_segment(self, segment):
        return _get_closest_point_in_segment(self, segment)

    def is_collinear(self, other):
        assert isinstance(other, Line2)
        d = other.v.magnitude_squared()
        assert d != 0
        u = ((self.x - other.p.x) * other.v.x + \
             (self.y - other.p.y) * other.v.y) / d
        if not other._u_in(u):
            u = max(min(u, 1.0), 0.0)

        line_vector_part = self -Point2(other.p.x + u * other.v.x, other.p.y + u * other.v.y)

        if not line_vector_part:  # Vector part is (0,0), so the point is collinear
            return True
        else:
            return False
    ...
    def connect(self, other):
        if isinstance(other, Point2):
            return self._connect_point2(other)
        elif isinstance(other, Line2):
            return self._connect_line2(other)
        elif isinstance(other, Circle):
            return self._connect_circle(other)

    def _connect_point2(self, other):
        EPSILON = sys.float_info.epsilon  # Added by Carlos since EPSILON was undefined
        # EPSILON = 1e-8

        if (self -other).magnitude_squared() > EPSILON:
    ...
    def affine_combination(self, other, weight=0.5):
        '''
        ...
        '''
        return Point2((1 - weight) * self.x + weight * other.x, (1 - weight) * self.y + weight * other.y)
...
class Line2(Geometry):
    ...
    def __init__(self, *args):
        '''
        Constructors:
        -- Point2 p, Vector2 v, float s (== p + s*v)
        -- Point2 p, Point2 p2:         (== p +  (p2-p))
        -- Point2 p, Vector2 v          (== p + v)
        -- Line2 l                      (== l.p + l.v)
        '''
        if len(args) == 3:
            assert isinstance(args[0], Point2) and \
                   isinstance(args[1], Vector2)
            self.p = args[0].copy()
            self.v = args[1] * float(args[2]) / abs(args[1])
        elif len(args) == 2:
    ...
    def _u_in(self, u):  # FIXME: What is this doing? It's always True!
        return True

    def intersect(self, other):
        if isinstance(other, Point2):
            return self._intersect_point2(other)
        if isinstance(other, Line2):
            return self._intersect_line2(other)
        elif isinstance(other, Circle):
            return self._intersect_circle(other)
    ...
    def _intersect_point2(self, other):
        return _intersect_point2_line2(other, self)
    ...
    def get_closest_point_on_line(self, point):
        return  _get_closest_point_on_line(point, self)

    def distance_normal(self, point):
        return _distance_normal(point, self)
...
class LineSegment2(Line2):
    ...
    def get_closest_point_in_segment(self, point):
        return _get_closest_point_in_segment(point, self)
...
def _intersect_point3_line3(P, L):
    return False  # TODO

def _intersect_point3_sphere(P, S):
    return abs(P - S.c) <= S.r

def _intersect_point3_plane(P, pln):
    return False  # TODO
...
def _intersect_line3_plane(L, P):
    '''
    @brief it can compute the intersection of a line and a plane
    @return the Point3 of intersection if any. Otherwise, None is return
    '''
    d = P.n.dot(L.v)
    if not d:
        # Parallel
    ...
    if not L._u_in(u):
        ...
        inter_vec = L.p + u * L.v
        # Forcing intersection not be a Vector3, but a Point3 instead! (needed by Carlos)
        int_pt = Point3(inter_vec.x, inter_vec.y, inter_vec.z)
        return int_pt
...
def _intersect_line3_line3(A, B):
    ...
    sol = _connect_line3_line3(A, B)
    # WISH: Ray3 and LineSegment3 would like to be able to know
...
class Point3(Vector3, Geometry):
    def __repr__(self):
        return 'Point3(%.6f, %.6f, %.6f)' % (self.x, self.y, self.z)

    def intersect(self, other):
        # if isinstance(other, Line3):
        #    return self._intersect_line3(other)
        if isinstance(other, Sphere):
            return self._intersect_sphere(other)
        # elif isinstance(other, Plane):
        #    return self._connect_plane(other)
    ...
    def connect(self, other):
        if isinstance(other, Point3):
            return self._connect_point3(other)
        elif isinstance(other, Line3):
            return self._connect_line3(other)
        elif isinstance(other, Sphere):
            return self._connect_sphere(other)
        elif isinstance(other, Plane):
            return self._connect_plane(other)
    ...
    def affine_combination(self, other, weight=0.5):
        ...
        So... 0.33 is a point .33 of the way between self and other.
        '''
        return Point3((1 - weight) * self.x + weight * other.x,
                        (1 - weight) * self.y + weight * other.y,
                        (1 - weight) * self.z + weight * other.z,
                    )
...
class Line3(Geometry):
    ...
    def __init__(self, *args):
        '''
        Constructors:
        Point3 p, Vector3 v, float s        (== p+ s*v)
        Point3 p1, Point3 p2                (== p1 + (p2-p1))
        Point3 p, Vector3 v                 (== p+v)
        Line3 l                             (== l.p + l.v)
        '''
        if len(args) == 3:
            assert isinstance(args[0], Point3) and \
                   isinstance(args[1], Vector3)
            self.p = args[0].copy()
            self.v = args[1] * float(args[2]) / abs(args[1])
        elif len(args) == 2:
    ...
    def intersect(self, other):
        if isinstance(other, Line3):
            ...
            return self._intersect_plane(other)
    ...
    def _intersect_line3(self, other):
        return _intersect_line3_line3(self, other)
...
class Sphere(Geometry):
    ...
    def __init__(self, center=Point3(0.0, 0.0, 0.0), radius=1.0):
        '''
        Sphere Constructor. Default arguments are a unit sphere centered at the origin of coordinates.
        '''
    ...
    def intersect(self, other):
        if isinstance(other, Point3):
            return self._intersect_point3(other)
        elif isinstance(other, Line3):
            return self._intersect_line3(other)
    ...
    def __init__(self, *args):
        if len(args) == 3:
            ...
            if isinstance(args[0], Point3) and isinstance(args[1], Vector3):
                ...
                self.k = self.n.dot(args[0])
            elif isinstance(args[0], Vector3) and isinstance(args[1], (float, int)):
